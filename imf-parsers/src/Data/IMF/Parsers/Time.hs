...
module Data.IMF.Parsers.Time
  ( pTime
  )
where

import           Control.Applicative    (liftA3, (<|>))
import           Data.Attoparsec.Text   (Parser, count, digit, endOfInput, option, parseOnly)
...
instance HasParser ZonedTime where
    parser = pTime
...
--
pTime :: Parser ZonedTime
pTime = liftA3 (\d t z -> ZonedTime (LocalTime d t) z)
